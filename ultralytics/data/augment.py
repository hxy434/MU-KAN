...
class RandomCustomCover:
    def __init__(self, p_cover1=0.1, p_cover2=0.1) -> None:

        self.p_cover1 = p_cover1
        self.p_cover2 = p_cover2
    ...
    def __call__(self, labels):
        ...
        p_cover1 = random.random()
        p_cover2 = random.random()
        if p_cover1 < self.p_cover1:
            # 生成卡尺
            h = np.random.randint(image_height // 120, min(image_width // 20, image_height // 40))
            w = np.random.randint(h * 4, image_width // 5)
            x = np.random.randint(0, image_width - w)
            y = np.random.randint(0, image_height - h)
            # 随机颜色（黑色和白色为主）
            color_main = random.choice([np.random.randint(0, 50), np.random.randint(200, 255)])
            a = abs(color_main - 127) // 20
            color = (
                int(np.clip(color_main + np.random.randint(-a, a), 0, 255)),
                int(np.clip(color_main + np.random.randint(-a, a), 0, 255)),
                int(np.clip(color_main + np.random.randint(-a, a), 0, 255)),
            )

            # 生成随机数字
            number = str(np.random.randint(0, 1000)) + random.choice(["nm", "um", "pm"])
            # 随机选择字体
            fonts = [
            ...
            # 随机字体大小 (0.5到3.0之间)
            font_scale = random.uniform(0.5, 0.6)
            # 随机粗细 (1到5之间)
            thickness = random.randint(1, 2)
            text_size = cv2.getTextSize(number, font, font_scale, thickness)
            text_width, text_height = text_size[0]
            p = np.random.uniform(0, 1, 1)
            # 位于左边
            if p < 0.5:
                number_x = random.randint(x - int(text_width * 1.3), x - int(text_width * 1.1))
                number_y = random.randint(y + int(text_height * 0.8), y + int(text_height * 1.0))
            # 位于上方
            else:
                number_x = random.randint(x - int(text_width * 0.2), x + int(text_width * 0.2))
                number_y = random.randint(y - int(text_height * 1.5), y - int(text_height * 1.1))
        ...
        if p_cover2 < self.p_cover2:
            h = np.random.randint(image_height // 40, image_height // 10)
            w = np.random.randint(image_width // 40, image_width // 10)
            x = np.random.randint(0, image_width - w)
            y = np.random.randint(0, image_height - h)
            # 随机颜色（黑色和白色为主）
            color_main = random.choice([np.random.randint(0, 50), np.random.randint(200, 255)])
